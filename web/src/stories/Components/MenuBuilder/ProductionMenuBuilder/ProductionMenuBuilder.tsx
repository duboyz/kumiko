...
import { AddCategoryForm, CreateMenuForm } from '@/components'
import { SortableCategory } from '@/components'
...
export function ProductionMenuBuilder() {
  ...
  const { data: allMenuItemsData } = useRestaurantMenuItems(menu?.id || '')
  ...
  // Update menu title and description when data loads
  useEffect(() => {
    if (menu) {
  ...
  const addCategory = async (newCategory: CreateMenuCategoryCommand) => {
    ...
    try {
      await createCategoryMutation.mutateAsync({
        ...newCategory,
        restaurantMenuId: menu.id.toString(),
      })
    } catch (error) {
  ...
  const addMenuItem = async (categoryId: string) => {
    if (!newItem.name.trim() || !menu) return
  ...
  const confirmDeleteMenuItem = async (categoryItemId: string) => {
    try {
  ...
  const confirmEditMenuItem = (itemId: string) => {
  ...
  const sortedCategories = [...categories].sort((a: MenuCategoryDto, b: MenuCategoryDto) => a.orderIndex - b.orderIndex)
      ...
      <DeleteConfirmDialog
        ...
        onConfirmDelete={() => {
          if (deleteDialogOpen?.type === 'category') {
            ...
            confirmDeleteMenuItem(deleteDialogOpen.id)
          }
        }}
      />
      ...
      <EditWarningDialog
        ...
        onConfirmEdit={() => {
          if (editDialogOpen?.type === 'category') {
            ...
            confirmEditMenuItem(editDialogOpen.id)
          }
        }}
      />
    </div>
