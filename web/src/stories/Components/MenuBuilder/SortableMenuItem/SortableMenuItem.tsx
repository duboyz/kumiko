...
import { MenuItemDto, MenuCategoryItemDto, MenuCategoryDto } from '@shared'
...
export function SortableMenuItem({
  ...
  return (
    <TableRow ref={setNodeRef} style={style} className={`hover:bg-muted/30 ${isDragging ? 'shadow-lg z-50' : ''}`}>
      <TableCell>
      ...
      <TableCell>
        <div className="flex gap-1">
          {editingItem === item.id ? (
            <Button variant="ghost" size="sm" onClick={() => onUpdateMenuItem(category.id, item.id, {})}>
          ...
          <Button variant="ghost" size="sm" onClick={() => onDeleteItem(categoryItem.id)}>
