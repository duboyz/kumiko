...
export const menuApi = {
  ...
  getRestaurantMenus: async (restaurantId: string): Promise<ResponseData<GetRestaurantMenusData>> => {
    const { data } = await client.get<ApiResponse<GetRestaurantMenusData>>(`/api/restaurant-menus/restaurant/${restaurantId}`)
  ...
  getRestaurantMenuItems: async (restaurantMenuId: string): Promise<ResponseData<GetRestaurantMenuItemsData>> => {
    ...
    const { data } = await client.get<ApiResponse<GetRestaurantMenuItemsData>>(`/api/menu-items/restaurant-menu/${restaurantMenuId}`)
  ...
  getAllRestaurantMenuItems: async (restaurantId: string): Promise<ResponseData<GetAllRestaurantMenuItemsData>> => {
    ...
    const { data } = await client.get<ApiResponse<GetAllRestaurantMenuItemsData>>(`/api/menu-items/restaurant/${restaurantId}`)
  ...
  createRestaurantMenu: async (command: { name: string; description: string; restaurantId: string }): Promise<ResponseData<CreateRestaurantMenuData>> => {
  ...
  addMenuItemToCategory: async (command: AddMenuItemToCategoryCommand): Promise<ResponseData<AddMenuItemToCategoryData>> => {
  ...
  updateMenuCategoryItem: async (command: UpdateMenuCategoryItemCommand): Promise<ResponseData<MenuCategoryItemDto>> => {
    const { data } = await client.put<ApiResponse<MenuCategoryItemDto>>(`/api/menu-category-items/${command.id}`, command)
