...
public class SearchAddressHandler(
    ...
    ILogger<SearchAddressHandler> logger) : IQueryHandler<SearchAddressCommand, SearchAddressResult>
{
    ...
    public async Task<SearchAddressResult> Handle(SearchAddressCommand request, CancellationToken cancellationToken)
    {
        var results = await SearchAddressAsync(request.Request);
        return new SearchAddressResult(results);
    }

    private async Task<List<ResponseAddressSearch>> SearchAddressAsync(RequestSearchAddress request)
    {
        logger.LogInformation("Searching for address: {Query}", request.Query);
        ...
        var response = await httpClient.GetAsync(autocompleteUrl);
        response.EnsureSuccessStatusCode();

        var json = await response.Content.ReadAsStringAsync();
        logger.LogInformation("Google Places Autocomplete response: {Response}", json.Substring(0, Math.Min(500, json.Length)));
        ...
        {
            logger.LogInformation("Found {Count} predictions from Google Places API", autocompleteResponse.Predictions.Count);
        ...
        {
            logger.LogWarning("No predictions found from Google Places API for query: {Query}", request.Query);
        }
    ...
    private async Task<PlaceDetailsResult> GetPlaceDetailsAsync(string placeId, string apiKey)
    {
        try
        {
            ...
            var json = await response.Content.ReadAsStringAsync();
            logger.LogDebug("Place details response for {PlaceId}: {Response}", placeId, json.Substring(0, Math.Min(500, json.Length)));
            ...
            if (detailsResponse?.Result != null)
            {
                ...
                if (detailsResponse.Result.AddressComponents != null)
                {
                    ...
                    foreach (var component in detailsResponse.Result.AddressComponents)
                    {
                        ...
                        logger.LogDebug("  Types: {Types}, Long: {LongName}", string.Join(", ", types), longName);
        ...
        {
            logger.LogWarning(ex, "Failed to get details for place_id: {PlaceId}", placeId);
            return new PlaceDetailsResult();
        }
    }
    ...
    private class GoogleDetailsResponse
    {
